/*
 * SteeringWheelTest1.c
 *
 * Created: 10-Dec-15 1:07:53 PM
 *  Author: ant
 */ 
...
//void Initial(uchar mode);
void Display_Chess(unsigned char value1,unsigned char value2);
void Display_Picture(unsigned char pic[]);
void DrawString(uint x, uint y, uchar *pStr);
void DrawSingleAscii(uint x, uint y, uchar *pAscii);
void DrawSingleHz(uint x, uint y, uchar *pHz);
void Gray_test(void);
...
struct typFNT_GB16 HzLib[] = { /* 宋体 9小五 显示为12*16 */
...
struct typFNT_GB16 HzLib[] PROGMEM = { /* 宋体 9小五 显示为12*16 */
...
void Write_number(uchar *n,uchar k,uchar col)
{
	uchar i;
...
void Write_Data(unsigned char dat)
{ 
	PORTB |= 0b00100000;	//Set DC Pin High
	PORTD &= ~0b00000001;	//Set CS Pin Low
	PORTC &= ~0b10000000;	//Set Write Pin Low
	
	if(dat & 128) PORTB |= 0b00001000;
	else PORTB &= ~ 0b00001000;
	
	if(dat & 64) PORTC |= 0b01000000;
	else PORTC &= ~ 0b01000000;
	
	if(dat & 32) PORTC |= 0b00100000;
	else PORTC &= ~ 0b00100000;
	
	if(dat & 16) PORTB |= 0b00000100;
	else PORTB &= ~ 0b00000100;
	
	if(dat & 8) PORTD |= 0b10000000;
	else PORTD &= ~ 0b10000000;
	
	if(dat & 4) PORTB |= 0b00000010;
	else PORTB &= ~ 0b00000010;
	
	if(dat & 2) PORTB |= 0b00000001;
	else PORTB &= ~ 0b00000001;
	
	if(dat & 1) PORTB |= 0b10000000;
	else PORTB &= ~ 0b10000000;
	
	PORTC |= 0b10000000;	//set Write Pin High
	PORTD |= 0b00000001;	//Set CS Pin High
...
/*set gpio as outputs where needed
...
void Write_Instruction(unsigned char cmd)
{
	PORTB &= ~0b00100000;	//Set DC Pin Low
	PORTD &= ~0b00000001;	//Set CS Pin Low
	PORTC &= ~0b10000000;	//Set Write Pin Low
...
void Data_processing(uchar temp)  //turns 1byte B/W data to 4 bye gray data  
    ...
    if(j<len)
    {
    	...
    	Set_Column_Address(x);
 	    Write_Instruction(0x5c);
            ...
            for (k=0;k<2;k++)
            {
              Data_processing(HzLib[j].Msk[k+i*2]);
            } 
        }
    }
}
